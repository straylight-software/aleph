...
auto test_matmul() -> bool {
    ...
    if (!check_cuda_error(cudaMalloc(&d_a, sizeof(h_a)), "malloc A")) return false;
    if (!check_cuda_error(cudaMalloc(&d_b, sizeof(h_b)), "malloc B")) return false;
    if (!check_cuda_error(cudaMalloc(&d_c, sizeof(h_c)), "malloc C")) return false;
    ...
    // row 0: 1+2+3 = 6, row 1: 4+5+6 = 15, row 2: 7+8+9 = 24, row 3: 10+11+12 = 33
        ...
        for (int j = 0; j < N && passed; ++j) {
            if (h_c[i * N + j] != expected_row_sums[i]) {
                std::printf("  matmul C[%d,%d] = %f, expected %f\n",
                            i, j, h_c[i * N + j], expected_row_sums[i]);
                passed = false;
            }
        }
    }
...
auto test_reduction() -> bool {
    ...
    if (!check_cuda_error(cudaMalloc(&d_data, sizeof(h_data)), "malloc data")) return false;
    if (!check_cuda_error(cudaMalloc(&d_result, sizeof(float)), "malloc result")) return false;
