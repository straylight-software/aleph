...
int main(void) {
    ...
    // Compress
    if (compress((Bytef *)compressed, &comp_len, 
                 (const Bytef *)input, strlen(input) + 1) != Z_OK) {
    ...
    // Decompress
    if (uncompress((Bytef *)decompressed, &decomp_len,
                   (const Bytef *)compressed, comp_len) != Z_OK) {
