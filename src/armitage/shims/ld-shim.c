    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg, "-o") == 0 && i + 1 < argc) {
            ...
            pie = 1;
                   ...
                   strcmp(arg, "--no-as-needed") == 0 ||
                   strcmp(arg, "-z") == 0 ||
                   strncmp(arg, "-m", 2) == 0 ||
                   strncmp(arg, "--sysroot", 9) == 0) {
            /* Skip known flags */
            if (strcmp(arg, "-z") == 0 && i + 1 < argc) i++;
...
static void append_with_sep(char *buf, size_t bufsize, const char *str, char sep) {
...
static void read_object_metadata(const char *path) {
    ...
    for (int i = 0; i < num_syms; i++) {
        ...
        if (strncmp(name, "__armitage_", 11) != 0) continue;
