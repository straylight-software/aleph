    ...
    (dhallPath : _) -> do
        ...
        case result of
            ...
            Right target -> do
                ...
                let _drv = Dhall.targetToDerivation target
...
cmdShim args = case args of
    ...
    (arg : rest)
        | not ("-" `isPrefixOf` arg) && not (arg `elem` ["run", "read", "log", "env", "analyze"]) ->
    ...
    case linkInfo of
        Just li -> do
            ...
            when (length (Shim.liAllIncludes li) > 10) $
                putStrLn $
                    "    ... and " <> show (length (Shim.liAllIncludes li) - 10) <> " more"
            return ()
        Nothing -> do
            -- Try as object
        ...
        case result of
            ...
            Right ar -> do
                ...
                when (length (Shim.arSources ar) > 10) $
                    putStrLn $
                        "  ... and " <> show (length (Shim.arSources ar) - 10) <> " more"
                putStrLn ""
                ...
                when (length (Shim.arIncludes ar) > 5) $
                    putStrLn $
                        "  ... and " <> show (length (Shim.arIncludes ar) - 5) <> " more"
                putStrLn ""
        ...
        if null (DICE.arErrors result)
            then do
                ...
                forM_ (DICE.topoSort graph) $ \key -> do
                    ...
                    putStrLn $
                        "  "
                            <> T.unpack (DICE.aIdentifier action)
                            <> " ["
            ...
            then do
                ...
                if null (DICE.erFailed execResult)
                    then do
                        putStrLn ""
                        putStrLn "Outputs:"
                        forM_ (Map.toList $ DICE.erOutputs execResult) $ \(_key, paths) ->
                            forM_ paths $ \p ->
                                putStrLn $ "  " <> T.unpack p
    ...
    _ -> do
        ...
        case result of
            ...
            Right traceOutput -> do
                ...
                -- Check for interpreter invocations
                let fullTrace = Trace.parseFullTrace traceOutput
                    interpreterCalls = Trace.parseInterpreters cfg (Trace.ftExecves fullTrace)
                unless (null interpreterCalls) $
                    putStrLn $
                        "  " <> show (length interpreterCalls) <> " interpreter call(s)"
                putStrLn ""

                -- Analyze into build graph
                buildGraph <-
                    if null compiles && null links && not (null interpreterCalls)
                        then do
                            -- Use interpreter analysis
                            putStrLn "Analyzing interpreter execution..."
                            Trace.analyzeInterpreterTrace cfg fullTrace
                        else do
                            -- Use compiler analysis
                            when (null compiles && null links) $ do
                                hPutStrLn stderr "Warning: No compiler/linker/interpreter calls detected"
                                hPutStrLn stderr "Make sure the command does something traceable"
                            Trace.analyzeTrace cfg (compiles, links)
...
cmdUnroll args = case args of
    ...
    ("--help" : _) -> unrollUsage
    ("-h" : _) -> unrollUsage
    (flakeRef : _rest) | "-" `isPrefixOf` flakeRef -> unrollUsage
    (flakeRef : rest) -> do
...
unrollRec outDir maxDepth depth seen dryRun info
    ...
    | otherwise = do
        ...
        forM_ (diInputs info) $ \inputDrv -> do
            ...
            case inputInfo of
                Left _err -> putStrLn $ indent <> "  (failed: " <> takeBaseName inputDrv <> ")"
