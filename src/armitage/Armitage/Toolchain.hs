...
data Toolchain = Toolchain
    ...
    -- ^ C compiler flags
    , tcCXXFlags :: [Text]
    -- ^ C++ compiler flags (includes tcCFlags)
    , tcLDFlags :: [Text]
    -- ^ Linker flags
    , tcIncludePaths :: [Text]
    -- ^ -isystem paths
...
buildToolchainFromStdenv StdenvInfo{..} = do
    ...
    _hasGcc <- doesFileExist (T.unpack ccPath </> "gcc")
    ...
    let (ar, ld, nm, objcopy, strip) =
            if hasLlvmAr
                then
                    ( bintoolsPath <> "/llvm-ar"
                    , bintoolsPath <> "/ld.lld"
...
findClangResourceDir clangLibDir = do
    ...
    case exitCode of
        ExitSuccess -> do
            let versions =
                    filter (T.all (`elem` ("0123456789." :: String))) $
                        T.lines $
                            T.pack stdout
            case versions of
