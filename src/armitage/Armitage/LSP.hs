...
import System.IO (Handle, hFlush, hGetLine, hPutStr, stderr, stdin, stdout)
...
data LSPConfig = LSPConfig
    { lcBackendTimeout :: Int
    -- ^ Timeout for real LSP (microseconds)
    , lcFallbackEnabled :: Bool
    -- ^ Enable fallback mode
    , lcTraceCacheDir :: FilePath
    -- ^ Where to cache trace data
    , lcTreeSitterParsers :: FilePath
    -- ^ Tree-sitter parser directory
    , lcKnownLSPs :: Map Text Text
...
selectBackend cfg stateVar lang = do
    ...
    case Map.lookup lang states of
        Just bs
            | bsStatus bs == BackendHealthy
            , Just backend <- bsBackend bs ->
                pure backend
...
spawnLSP cmd = do
    ...
    case parts of
        [] -> error "spawnLSP: empty command"
        (exeT : argsT) -> do
            let exe = T.unpack exeT
                args = map T.unpack argsT
            (Just stdin', Just stdout', _, process) <-
                createProcess
                    (proc exe args)
                        { std_in = CreatePipe
                        , std_out = CreatePipe
    ...
    Shutdown -> do
        ...
        forM_ (Map.elems backends) $ \bs ->
            case bsBackend bs of
                Just (RealLSP ph _ _ _) -> terminateProcess ph
                _ -> pure ()
        pure ShutdownResult
...
forwardCompletion _uri _pos (RealLSP _ _stdin' _stdout' _) = do
...
forwardHover _uri _pos (RealLSP _ _stdin' _stdout' _) = do
    -- TODO: Implement LSP JSON-RPC forwarding
...
forwardDefinition _uri _pos (RealLSP _ _stdin' _stdout' _) = do
    -- TODO: Implement LSP JSON-RPC forwarding
...
fallbackCompletion fs uri _pos = do
    files <- readTVarIO (fsFiles fs)
    _graph <- readTVarIO (fsGraph fs)
...
fallbackDefinition fs uri _pos = do
    files <- readTVarIO (fsFiles fs)
    _graph <- readTVarIO (fsGraph fs)
    ...
    case Map.lookup uri files of
        ...
        Just fileState -> do
            ...
            let _importedUris = fsImports fileState
                _allFiles = Map.elems $ Map.restrictKeys files (Set.fromList (uri : _importedUris))
                _allSymbols = concatMap fsSymbols _allFiles
...
fallbackReferences _fs _uri _pos = do
...
-}
_updateDiagnosticsFromCompiler :: FallbackState -> Text -> Text -> IO ()
_updateDiagnosticsFromCompiler fs uri compilerStderr = do
    let diags = _parseCompilerDiagnostics uri compilerStderr
...
-}
_updateGraphFromTrace :: FallbackState -> Trace.FullTrace -> IO ()
_updateGraphFromTrace fs trace = do
    -- Extract which files were read (inputs) and written (outputs)
    let readPaths =
            [ Trace.faPath fa
            | fa <- Trace.ftFileAccesses trace
            , Trace.faMode fa == Trace.ModeRead
            , _isSourceFile (Trace.faPath fa)
            ]
        writes =
            [ Trace.faPath fa
            | fa <- Trace.ftFileAccesses trace
            , Trace.faMode fa `elem` [Trace.ModeWrite, Trace.ModeCreate]
            , _isOutputFile (Trace.faPath fa)
            ]

    -- For each output, record that it depends on all the reads
    forM_ writes $ \output -> do
        atomically $ modifyTVar (fsGraph fs) $ \g ->
            Map.insert output (Set.fromList readPaths) g

    -- Update import relationships in file state
    forM_ readPaths $ \inputFile -> do
        fileMap <- readTVarIO (fsFiles fs)
        case Map.lookup inputFile fileMap of
            Nothing -> pure () -- File not open in editor
            Just _fileState -> do
                -- This file was read during compilation - update its imports
                let otherReads = filter (/= inputFile) readPaths
                atomically $
                    modifyTVar (fsFiles fs) $
                        Map.adjust (\s -> s{fsImports = _nub (fsImports s ++ otherReads)}) inputFile

    -- Update diagnostics from compiler stderr
    let stderrOutput = Trace.ftStderr trace
    when (not $ T.null stderrOutput) $ do
        -- Try to attribute diagnostics to specific files
        forM_ readPaths $ \file -> do
            let diags = _parseCompilerDiagnostics file stderrOutput
            when (not $ null diags) $
                atomically $
                    modifyTVar (fsDiagnostics fs) (Map.insert file diags)

_isSourceFile :: Text -> Bool
_isSourceFile path =
    any
        (`T.isSuffixOf` path)
        [ ".rs"
...
_nub :: (Ord a) => [a] -> [a]
_nub = Set.toList . Set.fromList

{- | Parse compiler stderr into diagnostics
TODO: Add per-compiler parsers (rustc, clang, ghc, etc.)
-}
_parseCompilerDiagnostics :: Text -> Text -> [Diagnostic]
_parseCompilerDiagnostics uri stderrText =
...
methodToRequest method params = case method of
    ...
    "workspace/symbol" ->
        case params of
            Aeson.Object obj
                | Just (Aeson.String q) <- AesonKM.lookup "query" obj ->
                    WorkspaceSymbol q
