...
_defaultBuildConfig :: BuildConfig
_defaultBuildConfig =
    BuildConfig
        ...
        case (getString "builder", getObject "env", getObject "outputs") of
            (Just builder, Just envObj, Just outputsObj) -> do
                let args = case getList "args" of
                    ...
                    drvSystemValue = fromMaybe "x86_64-linux" (getString "system")
...
outputsToDhall m
  ...
  where
    entries =
        [ "`" <> k <> "` = " <> drvOutputToDhall v
        | (k, v) <- Map.toAscList m
        ]

-- | Render DrvOutput to Dhall
drvOutputToDhall :: DrvOutput -> Text
drvOutputToDhall DrvOutput{..} =
    T.concat
        [ "{ doHash = "
        , maybeTextToDhall doHash
        , ", doHashAlgo = "
        , maybeTextToDhall doHashAlgo
        , ", doPath = "
        , maybeTextToDhall (T.pack <$> doPath)
        , " }"
        ]
...
executeBuilder config drv env = do
    ...
    forM_ (Map.toList $ drvOutputs drv) $ \(name, _out) -> do
    ...
    when (bcVerbose config) $ do
        putStrLn $ "Builder output: " <> stdoutOutput
        putStrLn $ "Builder stderr: " <> stderrOutput
    ...
    case exitCode of
        ...
        ExitFailure code ->
            throwIO $ BuildFailed code (T.pack stderrOutput)
