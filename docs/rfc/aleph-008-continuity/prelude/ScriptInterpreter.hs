...
data Path
  ...
  | PathOut Text      -- $out-relative
  | PathDep Text Text -- Dependency output
  | PathTmp Text      -- $TMPDIR-relative
...
cmdToBash = \case
  ...
  CmdCopy{..} ->
    "cp " <> (if copyRecursive then "-r " else "") 
  ...
  CmdWrite{..} ->
    "cat > " <> quote (pathToBash writePath) <> " <<'DHALL_EOF'\n" 
             <> interpToBash writeContent <> "\nDHALL_EOF"
  
  CmdAppend{..} ->
    "cat >> " <> quote (pathToBash appendPath) <> " <<'DHALL_EOF'\n"
              <> interpToBash appendContent <> "\nDHALL_EOF"
  
  CmdSubstitute{..} ->
    let sedExprs = map (\(from, to) -> 
          "s|" <> escapeSed from <> "|" <> escapeSed (interpToBash to) <> "|g"
          ) subReplacements
    in "sed -i " <> T.intercalate " " (map (\e -> "-e " <> quote e) sedExprs) 
                 <> " " <> quote (pathToBash subFile)
  
  CmdUntar{..} ->
    "tar xf " <> quote (pathToBash untarArchive) 
              <> " -C " <> quote (pathToBash untarDest)
