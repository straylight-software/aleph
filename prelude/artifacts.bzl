...
# Wrapper to support wrapping `Artifact`s referencing paths behind external symlinks.
ArtifactExt = record(
    artifact = field(Artifact),
    # Optional sub-path to append when accessing the path.
    sub_path = field(str | None, None),
    # Returns the resolved path as a `cmd_arg()`, with the optional sub-path appended.
...
# specifying an `ArtifactExt` as its default output.
...
def single_artifact(dep: Artifact | Dependency) -> ArtifactOutputs:
    ...
    if DefaultInfo in dep:
        ...
        return ArtifactOutputs(
            ...
            nondebug_runtime_files = other_outputs,
