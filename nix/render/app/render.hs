...
import Render.Lint.Forbidden (findViolations, formatViolations)
import qualified Render.Nix.Parse as Nix
import qualified Render.Nix.Format as NixFmt
import qualified Render.Nix.Flake as Flake
import qualified Render.Nix.Module as Mod
import qualified Render.Nix.Layout as Layout
import qualified Render.Nix.Lint as Lint
import qualified Render.Nix.Scope as Scope
import qualified Render.Nix.Types
...
cmdFlake dir = do
  ...
  where
    ...
    prettyType = Render.Nix.Types.prettyType
    ...
    printModuleType rootDir graph path =
      case Map.lookup path (Mod.mgModules graph) of
        ...
        Just m -> do
          ...
          TIO.putStrLn $ T.pack relPath <> " : " <> Render.Nix.Types.prettyType (Mod.modType m)
